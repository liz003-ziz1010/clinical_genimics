...
HiFi Variant Calling Pipeline — Aligns PacBio HiFi long reads to a reference genome and calls variants (SNPs/Indels) using deep learning (Clair3).
RNA-seq Quantification Pipeline — Quantifies gene expression from short-read RNA-seq data using Salmon, with quality control via FastQC and MultiQC.
...
Push to the branch (git push origin feature/new-pipeline)
Open a Pull Request
License
...
Built with Nextflow, Singularity, and a lot of caffeine (^^)
